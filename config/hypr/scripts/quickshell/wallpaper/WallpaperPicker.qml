...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    ( matugen image "$FINAL_THUMB" --type scheme-monochrome --source-color-index 0 || true; bash "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        if (isVideo) {
            wallpaperCmd = `
                ...
                if [ "${escOutputs}" = "all" ]; then
                    mpvpaper -o 'loop --no-audio --hwdec=auto --profile=high-quality --video-sync=display-resample --interpolation --tscale=oversample --panscan=1.0' '*' "${escOriginal}" >> ${logFile} 2>&1 &
                else
                    IFS=',' read -ra MON_ARR <<< "${escOutputs}"
                    for mon in "\${MON_ARR[@]}"; do
                        mpvpaper -o 'loop --no-audio --hwdec=auto --profile=high-quality --video-sync=display-resample --interpolation --tscale=oversample --panscan=1.0' "\$mon" "${escOriginal}" >> ${logFile} 2>&1 &
                    done
                fi
            `;
        } else {
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( matugen image "${escThumb}" --type scheme-monochrome --source-color-index 0 || true; bash "${escReload}" || true ) &
        `;
